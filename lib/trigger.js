// yuyuko-helper · 触发词识别（纯函数，可单测）
//
// 为什么需要它：判定条件是两条 AND —— 消息 **@ 了机器人本人**，且去掉 @提及 后第一个词是 yuyuko。
// QQ Agent 的 OneBot 文本已经把 at 段还原成了 `@<群名片或QQ号>`，
// 所以实际收到的可能长这样：
//
//   "@老八(QQ:1000000001) yuyuko me"
//   "@老八(QQ:1) yuyuko ship 大和 recent 30"
//   "yuyuko me"                           ← 没 @ 机器人（requireAt 打开时不认领）
//   "  yuyuko   roll 日本 战列舰 10"
//   "@老八(QQ:1) 帮我看看 yuyuko me"        ← ❌ 触发词不在最前
//   "@机器人(QQ:1) @yuyuko me"             ← ❌ 再 @ 一次触发词也不认（@ 是"叫某人"的语法）
//
// 两条判定（AND）：
//   ① `requireAt` 打开时，消息必须 **@ 了机器人本人**（默认打开）
//   ② 去掉开头的 @提及 与空白后，**第一个词**必须是触发词
//
// ⚠️ 为什么坚持"触发词必须在最前"：群里聊到 yuyuko 是常态，
//    只要含 yuyuko 就触发会疯狂抢话。宁可不触发，也不能抢话。

/**
 * 提取开头的连续 @提及。
 *
 * 实现走"逐字符扫描"而不是一条大正则，原因见下面注释里记的两个真实坑：
 *   坑 1（指令丢失）：用惰性量词 /^@[^\s@]{0,64}?/ 时，"@老八(QQ:123) yuyuko 大和"
 ...
 *   command = 去掉触发词后的指令正文（不带 yuyuko 前缀，正好是 Hikari 要的格式）
 */
export function matchTrigger(text, keywords = ['yuyuko'], options = {}) {
  ...
  //    原因：那会让 `@机器人 @yuyuko me` 也认领，而 @ 是"叫某人"的语法，
  //    用它来当触发词属于意料之外的输入，宁可不认领。
  //    注意 `@机器人 yuyuko me` 并不依赖它 —— 那时 yuyuko 只是普通词，
  //    由下面 head 那条判断正常处理。
  const body = body0;
