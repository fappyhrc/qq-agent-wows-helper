 ...
 *
 * ⚠️ 用户在设置页改的值存在 `data/config.json` 的 **`skills['yuyuko-helper']`** 下
 *    （平台的 `getSkillConfig` 就是把 `config.skills[id]` 与 manifest 默认值合并后
 *    交给 `api.config()`）。顶层另有同名 `plugins` 段的写法是不存在的 ——
 *    若手工把配置写到别处，这里读到的就是 `DEFAULTS`，表现为"凭据没配"。
 */
...
export const DEFAULTS = {
  ...
  // 只保留 yuyuko（核验后的结论）：`@机器人 yuyuko me` 即查总水表，
  // 触发词**后面的内容**原样交给 Hikari-core-v2。
  // ⚠️ 上游自己的帮助页仍写 wws —— 那是上游的示例；本插件只认 yuyuko，照它把 wws 换成 yuyuko 即可。
  triggerKeywords: ['yuyuko'],
  ...
  // 一次 yuyuko 查询要经过 "yuyuko API + 浏览器端 Nunjucks 渲染 + 截图"，实测
  // （Python 3.14 / chromium / 热态）单次 5.2~13.1 秒；首次还需下载浏览器与
  // 船图缓存（约 150 秒）。钩子硬超时仅 5 秒，预取在普通机器上必然超时，
  // 只会让每次 @机器人 白等几秒再退回工具。默认交给 wows-query 工具执行。
  hookPrefetch: false,
