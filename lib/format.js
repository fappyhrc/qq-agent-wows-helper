...
export function buildContextNote({ command, data, autoSendImage = true, maxChars = 1600, includeData = true, requireAt = true } = {}) {
  ...
  if (hasImage) {
    lines.push(autoSendImage
      ...
      : '渲染图未发送（自动发送已关闭）。需要把图发出去时，调用 yuyuko-helper__wows-send-image 工具。');
  }
...
export function formatResultText(result, config = {}) {
  ...
  if (image) {
    if (image.oversized) {
      ...
      lines.push('渲染图尚未发送。需要发图就调用 yuyuko-helper__wows-send-image 工具（同一张图只会发一次，重复调用会被去重拦下）。');
    }
  } else {
